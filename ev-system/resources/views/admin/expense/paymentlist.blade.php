                                              ...
                                              <div class="form-group">
                                                ...
                                                <div class="col-lg-9">
                                                  <select id="heard" name="student_id" class="form-control" required>
                                                    ...
                                                    <option value="{{ $cat->id }}">{{ $cat->classes ? $cat->classes->title : '' }} - {{ $cat->name }}</option>
                                                    @endforeach
                                                  </select>
                                                </div>
                                              </div>
    ...
    <div id="hide-table">
            <table id="example1" class="table table-striped table-bordered table-hover dataTable no-footer"> 
                  ...
                  <tr>
                      ...
                      <td data-title="Student">{{ App\User::find($post->student_id) ? App\User::find($post->student_id)->name : ''  }}</td>
                      <td data-title="{{ trans('invoice_lang.invoice_total') }}">{{ $invoices->first()->amount }}</td>
                      <td data-title="{{ trans('invoice_lang.invoice_amount') }}">{{ $invoices->first()->amount_paid }}</td>
                      <td data-title="{{ trans('invoice_lang.invoice_status') }}">@if( $invoices->first()->status == 'unpaid')<button class="btn btn-danger btn-xs"></i>{{ $invoices->first()->status }}</button>
                        @elseif( $invoices->first()->status == 'paid')
                            <button class="btn btn-success btn-xs"></i>{{ $invoices->first()->status }}</button>
                        @endif
                        </td>
                      <td data-title="{{ trans('invoice_lang.invoice_date') }}">{{ $invoices->first()->date }}</td>
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="form-group">
                                                <label  class="col-lg-2 control-label">{{ trans('invoice_lang.invoice_date') }}</label>
                                                <div class="col-lg-9">
                                                    <input type="text" class="form-control" value="{{ $invoices && $invoices->where('id',$post->invoice_id)->first() ? $invoices->where('id',$post->invoice_id)->first()->date : '' }}" name="date"required>
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label  class="col-lg-2 control-label">{{ trans('invoice_lang.invoice_total') }}</label>
                                                <div class="col-lg-9">
                                                    <input type="text" class="form-control" value="{{$invoices &&  $invoices->where('id',$post->invoice_id)->first() ? $invoices->where('id',$post->invoice_id)->first()->amount : '' }}" name="total_amount"required>
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label  class="col-lg-2 control-label">{{ trans('invoice_lang.invoice_amount') }}</label>
                                                <div class="col-lg-9">
                                                    <input type="text" class="form-control" value="{{$invoices &&  $invoices->where('id',$post->invoice_id)->first() ? $invoices->where('id',$post->invoice_id)->first()->amount_paid : '' }}" name="amount"required>
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label class="col-lg-2 control-label">{{ trans('invoice_lang.invoice_status') }}
