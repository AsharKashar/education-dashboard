      ...
      <div class="row">
        ...
        <div class="col-lg-6">
          ...
          {{-- <div class="white-box cst-white-box">
            <h3 class="box-title">Attendance Statss (<?php// echo date('d').'-'.date('M').'-'.date('Y');?>)</h3>
            <ul class="basic-list">
              <?php 
              ...
              @foreach($stud->where('class_id',$class->id) as $stt)
                ...
                ?>
              @endforeach
              <li>{{ $class->title }}<span class="pull-right label-danger label">{{ $diff }}</span> <span class="pull-right label-success label">{{ $sum }}</span></li>
              @endforeach
            </ul>
          </div> --}}
        </div>
      ...
      <div class="col-sm-8">
          <div class="white-box cst-white-box2">
            ...
            <ul class="basic-list">
              ...
              $stud = App\Student::all();?>
              @foreach($classes as $class)
              <li>{{ $class->title }}<span class="pull-right label-success label">{{ $stud->where('class_id',$class->id)->count() }}</span></li>
              @endforeach
            </ul>
          </div>
        </div>
