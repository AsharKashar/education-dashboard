...
class User extends Authenticatable
{
    ...
    public static function get_users()
	{
		if(auth()->user()->admin_type == 'super'){
			...
			$users = User::where('school_id', auth()->user()->school_id)->orderBy('created_at','desc')->get();
		}
		return $users;
	}
    public static function get_user($role)
	{
		if(auth()->user()->admin_type == 'super'){
			$users = User::where('role', $role)->orderBy('created_at','desc')->get();
		}
		else{
			$users = User::where('school_id', auth()->user()->school_id)->where('role', $role)->orderBy('created_at','desc')->get();
		}
		return $users;
	}
