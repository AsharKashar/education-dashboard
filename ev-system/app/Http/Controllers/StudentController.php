...
class StudentController extends Controller
{
    ...
    {   
        ...
        $classes = Classes::where('school_id', auth()->user()->school_id)->orderBy('created_at','desc')->where('id',$id)->get();
    ...
    {   
        if(!Auth::user()->permission('is_student'))
			return redirect('dashboard');
        $tests = Test::where('class_id', auth()->user()->class_id)->orderBy('created_at','desc');
    ...
    {   
        ...
        $test = Test::find($id);
        if($test){
            $questions = TestQuestion::orderBy('created_at','asc')->where('test_id',$test->id)->get();
            if($questions->count() > 0){
                $res = TestResult::where('student_id',Auth::user()->id)->where('test_id',$id)->first();
                if ($res && $res->counter > $test->redo) {
                    $error = "Maximum number of retries reached";
                    return redirect('student/online_test')->with('error',$error);
                }
                if ($res && $res->status ==  1) {
                    $error = "Sorry, this test has been marked";
                    return redirect('student/online_test')->with('error',$error);
                }
    ...
    public function store_test(Request $request)
    {
       ...
       // dd($request->all());
        ...
        $gettest = Test::find($test_id);
        if($gettest){
            $questions = TestQuestion::orderBy('created_at','asc')->where('test_id',$test_id)->get();
            $answer = [];
            $types = $request->get('type');
            $options = $request->get('option');
            foreach ($questions as $key => $tq) {
                if ($tq->type == 'options'){
                    $tq->options = TestOption::where('question_id', $tq->id)->get();
                }
                $getID = $request->get('id')[$key];
                if(isset($types[$getID])){
                    if(isset($options[$getID])){
            ...
            $getdone = TestResult::where('student_id',$student->id)->where('test_id',$test_id)->first();
            if ($getdone) {
                $getdone->counter = $getdone->counter+1;
                $getdone->questions = json_encode($questions->toArray());
                $getdone->answer = json_encode($answer);
                $getdone->save();
            }
            else
            {
                ...
                $test->counter = 1;
                $test->questions = json_encode($questions->toArray());
                $test->answer = json_encode($answer);
                $test->save();
            }

            
            $success = "Test saved";
            return redirect('student/test_result/'.$student->id)->with('success',$success);
        }
        else{
            $error = "Test not found";
			return redirect('student/online_test')->with('error',$error);
