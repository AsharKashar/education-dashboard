...
class ApiController extends Controller
{
	...
	public function test_upload(Request $req)
	{
        ...
        if(count((array)$files) <= 2 && count((array)$files) > 0){
            $processes = array();
            $hypes = array();
            foreach($files as $key => $file){
				$unique_url = rand(11111111,99999999);
				$destination_path = public_path().'/media';
				$extension        = $file->getClientOriginalExtension();
				$file_name        = $unique_url.'.'.$extension;
				try{
            ...
            return response()->json(['errors'=>"no file is uploaded"], 401); 
        }
	}
	public function store_message(Request $req)
	{
		$receiver = User::where('email', $req->get('to'))->first();
		if($receiver){
			$messages = new Message();
			$messages->from = $req->get('from');
			$messages->to = $receiver->id;
			$messages->title = $req->get('title');
			$messages->body = $req->get('body');
			$messages->active = 0;
			$messages->save();
			return response()->json($messages, 200); 
		}
		else{
			return response()->json('Receiver not found', 401); 
		}
